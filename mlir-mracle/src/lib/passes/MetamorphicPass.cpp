...
static mlir_mracle::OutcomeRelation composeRelation(mlir_mracle::OutcomeRelation a,
...
// apply
static bool canApplyAfter(mlir_mracle::OutcomeRelation cur,
            ...
            for (int attempt = 0; attempt < kMaxTransformAttempts; ++attempt) {
                ...
                while (transformCounter < maxApply) {
                    ...
                    for (const TransformSpec *spec : transforms)
                        if (canApplyAfter(
                                aggregate.value_or(mlir_mracle::OutcomeRelation::Equality),
                                spec->relation))
