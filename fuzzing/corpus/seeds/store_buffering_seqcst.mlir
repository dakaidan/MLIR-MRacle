// store buffering (SB) litmus test, seq_cst, with r0/r1 made observable
// P0: atomic_store(x, 1, seq_cst); r0 = atomic_load(y, seq_cst)
// P1: atomic_store(y, 1, seq_cst); r1 = atomic_load(x, seq_cst)
// r0/r1 are written to shared telemetry locations z/w before the join, so the
// post-join return observes them. (r0, r1) = (0, 0) is forbidden under seq_cst.
module {
    ...
    omp.parallel {
      omp.sections {
        ...
        omp.section {
          ...
          omp.atomic.read %r1_mem = %x memory_order(seq_cst) : memref<1xi64>, memref<1xi64>, i64
