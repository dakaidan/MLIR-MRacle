...
Add headers under `include/mlir-mr/`, implementations under `lib/`, and tests under `tests/`. When the first implementation is added, change `mlir_mr` from an INTERFACE library to a normal STATIC library and adjust the CMake visibility keywords.
